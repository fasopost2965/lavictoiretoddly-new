...
conn.on('ready', () => {
    ...
    const cmd = `
        php ${basePath}artisan tinker --execute="
            try {
                \\Illuminate\\Support\\Facades\\Mail::raw('Test Titan Mail', function (\\$message) {
                    \\$message->to('fsore701@outlook.com')->subject('Diagnostic Titan Mail');
                });
                echo '===DIAGNOSTIC_SUCCESS===';
            } catch (\\Exception \\$e) {
                echo '===DIAGNOSTIC_ERROR===|' . \\$e->getMessage() . '|' . get_class(\\$e);
            }
        "
    `;
