...
conn.on('ready', () => {
  ...
  const filePath = 'domains/sore-sports.com/new/lang/fr/validation.php';
  const content = `<?php
...
return [
    ...
    'lte' => [
        ...
        'array' => 'Le tableau :attribute doit pas avoir plus de :value éléments.',
    ],
    'max' => [
...
`;
  ...
  conn.exec(\`echo "\${base64Content}" | base64 -d > \${filePath}\`, (err, stream) => {
    if (err) throw err;
    stream.on('close', () => {
      console.log('File written via base64: ' + filePath);
      conn.end();
    });
  });
}).on('error', (err) => {
